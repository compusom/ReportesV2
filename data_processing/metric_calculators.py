...
def _calcular_dias_activos_totales(df_combined):
    ...
    if df_combined is None or df_combined.empty:
        ...
        return results
    if 'Entrega' not in df_combined.columns:
        logger.warning("Adv: Col 'Entrega' no encontrada (días activos).")
        return results
    active_df=df_combined[df_combined['Entrega'].eq('Activo')].copy()
    if active_df.empty:
        logger.warning("Adv: No hay filas con estado 'Activo'.")
        return results
    if 'date' not in active_df.columns or not pd.api.types.is_datetime64_any_dtype(active_df['date']):
        logger.warning("Adv: Col 'date' inválida.")
        return results
    ...
    if 'Campaign' in active_df.columns:
        try:
            ...
            logger.info("Días activos calc. %s campañas.", len(days_camp))
        except Exception as e_camp:
            logger.error("Error días campaña: %s", e_camp)
    ...
    if 'AdSet' in active_df.columns and 'Campaign' in active_df.columns :
        try:
            ...
            logger.info("Días activos calc. %s AdSets (Campaign/AdSet).", len(results['AdSet']))
        except Exception as e_adset:
            logger.error("Error días AdSet: %s", e_adset)
    ...
    if 'Anuncio' in active_df.columns and 'Campaign' in active_df.columns and 'AdSet' in active_df.columns:
       try:
           ...
           logger.info("Días activos calc. %s Anuncios.", len(days_ad))
       except Exception as e_ad:
           logger.error("Error días Anuncio: %s", e_ad)
